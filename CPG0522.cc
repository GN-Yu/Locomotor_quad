...
static double phi[5]={0,0,0,0,0}; //swing phase indicator

double sgnl_overext(double phi, double l, double D)
{
    if(sin(phi)>0) return 0;
    else return pow(l/D,4);
}

double sgnl_unload(double phi, double load, double Gu)
{
    if(sin(phi)>0) return 0;
    else if(load>Gu) return 0;
    else if(load<=0) return 1000;
    else return 50;
}
...
int main(int argc,char** argv)
{
    ...
    int sw[5]={};
    double Omega[5]={}; //angular velocity of the phases
	...
	{
		ifstream("CPGini")>>xc>>yc>>vx>>vy>>theta>>omega>>Gpre
		>>xfl>>xfr>>xhl>>xhr
		>>yfl>>yfr>>yhl>>yhr
	...
	{
		ifstream("CPGslowini")>>xc>>yc>>vx>>vy>>theta>>omega>>Gpre
		>>xfl>>xfr>>xhl>>xhr
		>>yfl>>yfr>>yhl>>yhr
		>>phi[1]>>phi[2]>>phi[3]>>phi[4];
	...
	{
		ifstream("CPGfastini")>>xc>>yc>>vx>>vy>>theta>>omega>>Gpre
		>>xfl>>xfr>>xhl>>xhr
		>>yfl>>yfr>>yhl>>yhr
		>>phi[1]>>phi[2]>>phi[3]>>phi[4];
    ...
    for(int i=1;i<=4;i++) {sw[i]=(sin(phi[i])>0);}
...
//			cerr<<G[k1]<<'\t'<<G[k2]<<endl; return 1; 
		...
		for(int k=1;k<=4;k++) cout<<'\t'<<(sw[k]?k:0);
		cout<<'\t'<<deltal<<'\t'<<deltar;
		cout<<endl;
		...
		cerr<<"set object 1 circle front at "<<xfl<<','<<yfl<<" size "<<.2<<" fc \"brown\""<<" fs "<<(sw[1] ? "empty" : "solid")<<endl;
		cerr<<"set object 2 circle front at "<<xfr<<','<<yfr<<" size "<<.2<<" fc \"blue\""<<" fs "<<(sw[2] ? "empty" : "solid")<<endl;
		cerr<<"set object 3 circle front at "<<xhl<<','<<yhl<<" size "<<.2<<" fc \"red\""<<" fs "<<(sw[3] ? "empty" : "solid")<<endl;
		cerr<<"set object 4 circle front at "<<xhr<<','<<yhr<<" size "<<.2<<" fc \"green\""<<" fs "<<(sw[4] ? "empty" : "solid")<<endl;
        ...
        for(int k=1;k<=4;k++) Omega[k]=M_PI/Tswc;
        double A=Omega[1], B=Omega[1];

        phi[1]+=Omega[1]+sgnl_overext(phi[1],d[1],DM[1])+sgnl_unload(phi[1],G[1],Gu)+A*sin(phi[1]-phi[2])+B*sin(phi[1]-phi[3]);
        phi[2]+=Omega[2]+sgnl_overext(phi[2],d[2],DM[2])+sgnl_unload(phi[2],G[2],Gu)+A*sin(phi[2]-phi[1])+B*sin(phi[2]-phi[4]);
        phi[3]+=Omega[3]+sgnl_overext(phi[3],d[3],DM[3])+sgnl_unload(phi[3],G[3],Gu)+A*sin(phi[3]-phi[4])+B*sin(phi[3]-phi[1]);
        phi[4]+=Omega[4]+sgnl_overext(phi[4],d[4],DM[4])+sgnl_unload(phi[4],G[4],Gu)+A*sin(phi[4]-phi[3])+B*sin(phi[4]-phi[2]);
		...
		// {
		// 	if(k==1 && (sw[2]==1 || sw[3]==1)) {continue;}
		// 	if(k==2 && (sw[1]==1 || sw[4]==1)) {continue;}
		// 	if(k==3 && (sw[1]==1 || sw[4]==1)) {continue;}
		// 	if(k==4 && (sw[2]==1 || sw[3]==1)) {continue;}
		// 	sw[k]=1;
		// 	tswpre[k]=t;
		// 	flcont++;
        ...
        // if(flcontpre==0) for(int k=1;k<=4;k++) if(sw[k]) if(t-tswpre[k]>=Tswc) 
		// {
		// 	sw[k]=0;
        ...
        for(int k=1;k<=4;k++) {sw[k]=(sin(phi[k])>0);}
	...
	if(newini==1)
	{	
		ofstream("CPGini")<<xc<<'\t'<<yc<<'\t'<<vx<<'\t'<<vy<<'\t'<<theta<<'\t'<<omega<<'\t'<<Gpre<<endl
	...
	{	
		ofstream("CPGslowini")<<xc<<'\t'<<yc<<'\t'<<vx<<'\t'<<vy<<'\t'<<theta<<'\t'<<omega<<'\t'<<Gpre<<endl
		<<xfl<<'\t'<<xfr<<'\t'<<xhl<<'\t'<<xhr<<endl
		<<yfl<<'\t'<<yfr<<'\t'<<yhl<<'\t'<<yhr<<endl
		<<phi[1]<<'\t'<<phi[2]<<'\t'<<phi[3]<<'\t'<<phi[4]<<endl;
	}
	...
	{	
		ofstream("CPGfastini")<<xc<<'\t'<<yc<<'\t'<<vx<<'\t'<<vy<<'\t'<<theta<<'\t'<<omega<<'\t'<<Gpre<<endl
		<<xfl<<'\t'<<xfr<<'\t'<<xhl<<'\t'<<xhr<<endl
		<<yfl<<'\t'<<yfr<<'\t'<<yhl<<'\t'<<yhr<<endl
		<<phi[1]<<'\t'<<phi[2]<<'\t'<<phi[3]<<'\t'<<phi[4]<<endl;
	}
