...
double DM[5]={0,L,L,1*L,1*L};	//max leg length
...
double load_calc(int swing[])
{
	...
	int nsw=0;
    for(int i=1;i<=4;i++) if(swing[i]) nsw++;	//swing legs count
	...
	else if(nsw==1)
	{
		...
		for(int k=1;k<=4;k++) if(swing[k]) for(int i=1;i<=4;i++) G[i]=F[i][k];
	}
	else if(nsw==2)
	{ 
		int k1=1; while(swing[k1]) k1++;
		double x1=xfh[k1],y1=yfh[k1];
		int k2=k1+1; while(swing[k2]) k2++;
		...
		Gtot=1-z*z/H/H;
		if(Gtot<0) {return -1;}
		Gtot=(sqrt(Gtot)-vz*vz/H/g<0? 0:sqrt(Gtot)-vz*vz/H/g);
...
int main(int argc,char** argv)
{
	...
	if(ini==0)
	{
		...
		xfl=xfl0; yfl=yfl0-.07;
		xfr=xfr0-.15; yfr=yfr0;
		xhl=xhl0-.1; yhl=yhl0-.1;
		xhr=xhr0+.02; yhr=yhr0;
	}
	else
	{
	...
	for(double t=0;t<=T;t+=dt)
	{
		...
		total_load=load_calc(sw);
		if(total_load<0) break;
		
		for(int i=1;i<=4;i++) {load[i]=G[i];}        
		...
		{
			sw[k]=0;
            double tottemp=load_calc(sw);
			if(tottemp<0 || G[k]<0) sw[k]=1;
		...
		if(total_load!=0 && -(total_load-Gpre)/dt>kv)
		{
			...
			sw[kmax]=0;
            double tottemp=load_calc(sw);
			for(int i=1;i<=4;i++) if(tottemp<0 || G[i]<0) sw[kmax]=1;
		...
		for(int k=1;k<=4;k++) if(swpre[k] && !sw[k])	//stop swing tracker
		{
			...
			// if(stridepre!=0) out_timers<<t<<'\t'<<vv<<'\t'<<k<<'\t'<<ttsw[k]<<'\t'<<-1<<'\t'<<(100*ttst[k]/stridetime)<<endl;
		}
		for(int k=1;k<=4;k++) if(!swpre[k] && sw[k])	//start swing tracker
		{
			ttst[k]=t-ttstpre[k];
			ttswpre[k]=t;
			if(stridedata_out==1 && stridepre!=0) out_timers<<t<<'\t'<<F0<<'\t'<<(k==1?1:0)<<'\t'<<(100*ttst[k]/stridetime)<<endl;
			// if(stridepre!=0) out_timers<<t<<'\t'<<vv<<'\t'<<k<<'\t'<<-1<<'\t'<<ttst[k]<<'\t'<<(100*ttst[k]/stridetime)<<endl;
		}
