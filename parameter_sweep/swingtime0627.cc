...
int main(int argc,char** argv)
{
	...
	for(int i=1;i<argc;i++)
	{
		...
		else if(strcmp(argv[i],"-out")==0) strcpy(fdur,argv[++i]);
		else return 1;
	}
...
//			cerr<<G[k1]<<'\t'<<G[k2]<<endl; return 1; 
		...
		int iter_nonneg;
		iter_nonneg=1;
		while(G[iter_nonneg]>=0 && iter_nonneg<4) iter_nonneg++;
		if(iter_nonneg<4) {if(t-fallpre>0.1) break;}
		else fallpre=t;
		...
		for(int k=1;k<=4;k++) if(swpre[k] && !sw[k])	//stop swing tracker
		{
			...
			// if(stridepre!=0) out_timers<<t<<'\t'<<F0<<'\t'<<0<<'\t'<<100*ttsw[k]/stridetime<<endl;
		}
		for(int k=1;k<=4;k++) if(!swpre[k] && sw[k])	//start swing tracker
		{
			ttst[k]=t-ttstpre[k];
			ttswpre[k]=t;
			if(stridepre!=0) out_timers<<t<<'\t'<<F0<<'\t'<<(k==1?1:0)<<'\t'<<(100*ttst[k]/stridetime)<<endl;
		}
