...
double Tswc;	//swing time parameter
...
//			cerr<<G[k1]<<'\t'<<G[k2]<<endl; return 1; 
		...
		cerr<<"set arrow 11 from "<<xh<<','<<yh<<" to "<<xf<<','<<yf<<((nsw==3)? "lc \"red\"":"lc \"black\"")<<" lt -1 lw 5 nohead"<<endl;
		cerr<<"plot \"tswdat\" u 2:3 w d"<<endl;

		
		static double tswpre[5]={};
		for(int k=1;k<=4;k++) tsw[k]=0;
		double P=1, DM[5]={0,L,L,L,L};
		flcont=0;
		...
		{
			sw[k]=1;				//start swinging for nonsupport or overextension
			//if(k<=2) {sw[k+2]=0;}	//stop hind swinging if same side front swings
			tswpre[k]=t;
			flcont++;
		}
		...
		{
			sw[k]=0;	//stop swing a leg after specific time
		}
