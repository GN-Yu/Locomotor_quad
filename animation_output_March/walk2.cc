...
int main(int argc,char** argv)
{
	...
	for(double t=0;t<=T;t+=dt)	//step cycles
	{
		...
		int nsw=0;	//swing legs count
		for(int i=1;i<=4;i++) if(sw[i]) nsw++;
		
		double G[5]={},Gtot=0;	//loads of each foot and the total load
		double Zx=0,Zy=0;
...
//		if(!fl[1] && !fl[2] && !fl[3] && !fl[4]) break;
		...
		cerr<<"set out \"tmp"<<int(t/dt)<<".png\""<<endl;
		...
		for(int k=1;k<=4;k++) if(!sw[k]) if((GP[k]>=Gu && G[k]<Gu) || d[k]>DM[k]) //start swinging for nonsupport or overextension
		{
			sw[k]=1;
			tswpre[k]=t;
			flcont++;
		...
		if(flcontpre==0 && ((Gpre<Gv && GS<Gpre) || nsw==3))
		{
			int kmax=0;
		...
		for(int k=1;k<=4;k++) if(swpre[k] && !sw[k])	//stop swing
		{
			ttstpre[k]=t; 
			ttsw[k]=t-ttswpre[k];
		...
		for(int k=1;k<=4;k++) if(!swpre[k] && sw[k])	//start swing
		{
			ttst[k]=t-ttstpre[k]; 
			ttswpre[k]=t;
	...
	return 0;
		...
		// double bodyc=(yfl0-yhl0)/(xfl0-xhl0);
		// double vtoa=(vx*(yhr0-yfl0)+vy*(xfl0-xhr0))/w;
		// double vtob=(vx*(yhl0-yfr0)+vy*(xfr0-xhl0))/w;
		// double ctoa=(xc-xhr0)*(yhr0-yfl0)+(yc-yhr0)*(xfl0-xfr0);
		// double ctob=(xc-xhl0)*(yhl0-yfr0)+(yc-yhl0)*(xfr0-xfl0);
		// double xhr1,yhr1,xfl1,yfl1;
		// double xhl1,yhl1,xfr1,yfr1;

		// if(ctoa*vtoa>=0)
		// {
		// 	xhr1=(vtoa-(bodyc*xhr0+yc-yhr0)*(xfl0-xfr0)-xc*(yhr0-yfl0))/(yfl0-yhr0-bodyc*(xfl0-xhr0));
		// 	yhr1=yhr0-bodyc*(xhr0-xhr1);
		// 	xfl1=xfl0-xhr0+xhr1;
		// 	yfl1=yfl0-yhr0+yhr1;
		// }
		// else 
		...
		// double xhl1=(vtob-(bodyc*xhl0+yc-yhl0)*(xfr0-xfl0)-xc*(yhl0-yfr0))/(yfr0-yhl0-bodyc*(xfr0-xhl0));
		// double yhl1=yhl0-bodyc*(xhl0-xhl1);
		// double xfr1=xfr0-xhl0+xhl1;
		// double yfr1=yfr0-yhl0+yhl1;