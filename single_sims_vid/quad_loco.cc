...
int main(int argc,char** argv)
{
	...
	if(ani_out)
	{
		cerr<<"set term png size 1000,1000"<<endl;
	...
	for(double t=0;t<=T;t+=dt)
	{
		...
		if(ani_out && int(t/dt)%int(DT/dt)==0)
		{
			...
			if(!sw[1] && !sw[2]) cerr<<"set arrow 1 from "<<xfl<<','<<yfl<<",0 to "<<xfr<<','<<yfr<<",0 nohead ls 0"<<endl;
			if(!sw[1] && !sw[3]) cerr<<"set arrow 2 from "<<xfl<<','<<yfl<<",0 to "<<xhl<<','<<yhl<<",0 nohead ls 0"<<endl;
			if(!sw[1] && !sw[4]) cerr<<"set arrow 3 from "<<xfl<<','<<yfl<<",0 to "<<xhr<<','<<yhr<<",0 nohead ls 0"<<endl;
			if(!sw[2] && !sw[3]) cerr<<"set arrow 4 from "<<xfr<<','<<yfr<<",0 to "<<xhl<<','<<yhl<<",0 nohead ls 0"<<endl;
			if(!sw[2] && !sw[4]) cerr<<"set arrow 5 from "<<xfr<<','<<yfr<<",0 to "<<xhr<<','<<yhr<<",0 nohead ls 0"<<endl;
			if(!sw[3] && !sw[4]) cerr<<"set arrow 6 from "<<xhl<<','<<yhl<<",0 to "<<xhr<<','<<yhr<<",0 nohead ls 0"<<endl;
			...
			cerr<<"set out \"pics/tmp"<<int(t/DT)<<".png\""<<endl;
