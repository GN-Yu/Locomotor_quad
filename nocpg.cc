...
//			Fx[k]=FL*cos(theta); Fy[k]=FL*sin(theta); 
		...
		for(int k=1;k<=4;k++)
		{
			...
			MA+=kr*(phi[k]-phpre[k])/dt; //+kr*kr/4/I*phi[k];
			phpre[k]=phi[k];
		}
	...
	if(int(t/dt)%int(DT/dt)==0)
	{
		...
		for(int i=1;i<=4;i++) 
        {
            cerr<<"set object circle front at "<<xfh[i]<<','<<yfh[i]<<",0 size "<<.2
                <<" fc \""<<colors[i]<<"\" fs "<<(sw[i] ? "empty" : "solid")<<endl;
            // cerr<<"set arrow from "<<xfh0[i]<<','<<yfh0[i]<<",1 to "<<xfh[i]<<','<<yfh[i]<<",0 nohead"<<endl;
        }
//		cerr<<"set object 5 circle front at "<<xc<<','<<yc<<" size "<<.2<<" fc \"black\""<<endl;
		...
		for(int i=1;i<=4;i++)
		{
			for(int k=1;k<=4;k++) if(!sw[i] && !sw[k])
				cerr<<"set arrow from "<<xfh[i]<<','<<yfh[i]<<",0 to "<<xfh[k]<<','<<yfh[k]<<",0 nohead"<<endl;
			cerr<<"set arrow from "<<xfh0[i]<<','<<yfh0[i]<<",1 to "<<xfh[i]<<','<<yfh[i]<<",0 nohead"<<endl;
		}

		cerr<<"set arrow from "<<xh<<','<<yh<<",1 to "<<xf<<','<<yf<<",1 lt -1 lw 5 nohead"<<endl;
		cerr<<"set arrow from "<<xhl0<<','<<yhl0<<",1 to "<<xhr0<<','<<yhr0<<",1 lt -1 lw 3 nohead"<<endl;
		cerr<<"set arrow from "<<xfl0<<','<<yfl0<<",1 to "<<xfr0<<','<<yfr0<<",1 lt -1 lw 3 nohead"<<endl;
		
		cerr<<"set xrange ["<<(xc-20)<<':'<<(xc+20)<<']'<<endl;
		cerr<<"set yrange ["<<(yc-20)<<':'<<(yc+20)<<']'<<endl;
        cerr<<"set zrange [0:5]"<<endl;
